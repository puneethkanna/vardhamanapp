...
def cgpa(rno, pas):
	...
	i=c.index(t)
	m = c[i+33]+c[i+34]+c[i+35]
	try:
		return(m)
	except IndexError:
		return("Something went wrong send the report to neerukondapuneeth@gmail.com stating the issue, with your rollno")
#cgpa()
def sgpa(rno,pas,semid):
	...
	for i in range(13, 100):
		t = "Semester Grade Point Average"
#		print(c[i+1])
#		c.remove("Semester Grade Point Average")
		index={"1":"13","2":"17","3":"21","4":"25","5":"29","6":"33","7":"37","8":"41"}
		ii = index[str(i+4)]
		print(str(th[1].text.strip())+":"+str(td[7].text.strip())+"\n"+str(th[int(ii)].text.strip()))
